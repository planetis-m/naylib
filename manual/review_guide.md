...
Functions related to text management, file operations, compression, and encoding are **always** excluded from the wrapper.
...
## Step 6: Do Not Modify Autogenerated Files

The file `src/raylib.nim` is autogenerated and overwritten every time wrappers are regenerated.

- **Never edit `src/raylib.nim` directly** when fixing compile issues or adding functionality.
- Instead, make changes in `tools/wrapper/snippets/` and the corresponding `*.cfg` files.
- Snippets define insertable sections such as module headers, extra types, or additional functions. These are the correct place for custom code that must persist across regenerations.
