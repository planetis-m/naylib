...
const
  RayLatestCommit = "d66692149bb0ba507be969971408fc82a6843194"
...
proc fetchLatestRaylib =
  if not dirExists(rayDir):
    direSilentShell "Cloning raysan/raylib...",
        "git clone --depth 10 https://github.com/raysan5/raylib.git", rayDir
  withDir(rayDir):
...
proc buildLatestRaylib(platform: string, wayland = false) =
  ...
  withDir(rayDir / "src"):
    ...
    direSilentShell "Building raylib static library...", exe, "clean &&",
        exe, "PLATFORM=" & platform, if wayland: "USE_WAYLAND_DISPLAY=TRUE" else: "", "-j4"
